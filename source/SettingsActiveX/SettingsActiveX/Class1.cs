...
namespace SettingsActiveX
{
    ...
    public class pdf_Reader : _InterfaceSettings
    {
        public Dictionary<string, string> getSettings()
        {
            ...
            int countLevels = 1; // Количество уровней 

            // Получаем настройки с листов документа (счет начинается с 1)
            for (int numLevel = 0; numLevel < countLevels; numLevel++)
            {
                Excel.Worksheet ObjWorkSheet = (Excel.Worksheet)ObjWorkBook.Sheets[numLevel + 1]; // Переходим к листу
